...
type Item = {
  item_id: number; categoria: string; pregunta: string; orden: number;
  escala_min: number; escala_max: number;
};

/** ===== Formulario genérico por rol ===== */
function FormByRole({ role, slug, target = null, title }: {
  role: Rol; slug: string; target?: 'docente'|'coord'|null; title: string;
}) {
  const [items, setItems] = useState<Item[] | null>(null);
  const [loading, setLoading] = useState(true);
  const [vals, setVals] = useState<Record<number, number>>({});
  const [modalidad, setModalidad] = useState(''); const [cursoId, setCursoId] = useState('');
  const [docenteId, setDocenteId] = useState(''); const [coordAsigId, setCoordAsigId] = useState('');
  const [loMejor, setLoMejor] = useState(''); const [aMejorar, setAMejorar] = useState('');
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const { data, error } = await supabase.rpc('api_items_autorizado', { p_rol: role, p_slug: slug });
      if (!on) return;
      if (error) { console.error(error); setItems([]); } else { setItems((data ?? []) as Item[]); }
      setLoading(false);
    })();
    return () => { on = false; };
  }, [role, slug]);
  ...
  const setValor = (id: number, v: number | string) => setVals(p => ({ ...p, [id]: Number(v) }));

  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); setMsg(null);
    const payload: any = {
      rol: role, modalidad: modalidad || null, curso_id: cursoId || null,
      ...
      no_aplica: false, respuestas: vals, lo_mejor: loMejor || null, a_mejorar: aMejorar || null,
    };
    ...
    <section className="card space-y-6">
      ...
      <form onSubmit={onSubmit} className="space-y-6">
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          {target === 'coord' && (<div className="sm:col-span-2">
                    ...
                    {Array.from({ length: it.escala_max - it.escala_min + 1 }).map((_, i) => {
                      ...
                      return (
                        <label key={v} className="inline-flex items-center gap-1">
                          <input type="radio" name={`item-${it.item_id}`} value={v}
                            checked={vals[it.item_id] === v} onChange={e=>setValor(it.item_id, e.target.value)} />
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <div><label className="label">Lo mejor</label>
            <textarea className="input" rows={3} value={loMejor} onChange={e=>setLoMejor(e.target.value)} />
          </div>
          <div><label className="label">Aspectos a mejorar</label>
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 2) 🔒 CLAMP: si NO es @uce.edu.ec ⇒ SOLO Estudiantes (sin consultar roles)
      if (email && !email.endsWith('@uce.edu.ec')) {
        setRoles(['estudiante']);
        return;
      }

      // 3) Institucional: pedir roles reales
      const { data, error } = await supabase.rpc<Rol[]>('api_current_roles');
      if (!alive) return;
      if (error) { console.error(error); setRoles(['estudiante']); } else { setRoles((data ?? []) as Rol[]); }
    })();
    return () => { alive = false; };
  }, []);
  ...
  // Debug opcional: pon NEXT_PUBLIC_SHOW_ROLE_DEBUG=1 en Vercel si quieres ver esto
  ...
  return (
    <main className="space-y-10">
      ...
      {!onlyStudent && roles.includes('coord_asignatura') && (
        <FormByRole
          role="coord_asignatura" slug="/eval/coord-asig" target="docente"
      ...
      {!onlyStudent && roles.includes('coord_nivelacion') && (
        <>
          <FormByRole
            role="coord_nivelacion" slug="/eval/coord-nivel-docentes" target="docente"
            title="EVALUACIÓN (Coordinación de Nivelación → docentes)"
          />
          <FormByRole
            role="coord_nivelacion" slug="/eval/coord-nivel-coord" target="coord"
